...
public class DashboardTest {
    ...
    public void shouldHaveMenu(FxRobot robot) {
        ...
        robot.lookup(".menu-bar").queryAs(MenuBar.class).getMenus().forEach(menu -> {
            ...
            verifyThat(menu.getItems().get(2), MenuItemMatchers.hasText("Revenues"));
        });
    }
    ...
    public void shouldChangeStageWhenClickOnMenu(FxRobot robot) throws TimeoutException {
        ...
        WaitForAsyncUtils.waitFor(2, TimeUnit.SECONDS, () -> robot.lookup("Revenues").match(NodeQueryUtils.isVisible()).tryQuery().isPresent());

        robot.clickOn("Revenues", Motion.VERTICAL_FIRST, MouseButton.PRIMARY);
    ...
    public void shouldHaveCharts(FxRobot robot) {
        ...
        assertThat(robot.lookup("#barChart").queryAs(BarChart.class).getTitle(), equalTo("Dépense VS Revenu"));
    }
}
